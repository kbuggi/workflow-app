        ...
        try:
            ...
            if not os.path.exists(self.local_folder):
                ...
                try:
                    ...
                    repo = git.Repo(
                        self.local_folder
                    )  # This raises an error if it's not a valid Git repo
                    ...
                    for info in result:
                        print(
                            f"Ref: {info.ref.name}"
                        )  # The branch or ref that was updated
                        print(
                            f"Commit: {info.commit}"
                        )  # The commit hash after the pull
                        print(
                            f"Flags: {info.flags}"
                        )  # Flags indicating the type of update
                    ...
                    msg = (
                        f"Invalid repository. Cloning new repo for {self.repo_url} ..."
                    )
                    print(msg)
                    self.update_signal.emit(msg)
                    git.Repo.clone_from(self.repo_url, self.local_folder)
