...
class GridCell(QGraphicsRectItem):
    def __init__(
        ...
        if (
            contains_emoji(label) and len(label) < 4
        ):  # left align & bottom align an individual emoji; bump fontsize
            font = QFont("Arial", 24)
    ...
    # Behaviour when a cell is left or right clicked;
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            menu = QMenu()
            # Add options (IRL would be different for Stream, Task, ...)
            move_up_action = menu.addAction("Move Up")
            edit_action = menu.addAction("Edit")
            add_task_action = menu.addAction("Add New Task")
            add_stream_action = menu.addAction("Add New Stream")
            ...
            add_stream_action.triggered.connect(self.add_stream)
            move_down_action.triggered.connect(self.move_down)

            menu.exec(event.screenPos())

    def move_up(self):
        print(f"Move Up called on {self.label}")

    def move_down(self):
        print(f"Move Down called on {self.label}")

    def edit(self):
        print(f"Edit called on {self.label}")

    def add_task(self):
        print(f"Add New Task called on {self.label}")

    def add_stream(self):
        print(f"Add New Stream called on {self.label}")
    ...
    def update(self, label, data):
        ...
        for key, value in data.items():
            if isinstance(value, dict):  # Expand dictionary as level 1
                ...
                )  # Set colour of level 1 key for visibility
...
class GridController(QWidget):
    ...
    def populate_cell(
        self, column, row, width, text_colour, background_colour, label, dictionary
    ):
        if isinstance(column, str):
            column = self.COLUMN_MAP.get(
                column, 0
            )  # map these strings; otherwise assume they give number {'Left' : 1, 'Middle' : 2, 'Right' : 3}
    ...
    def reset_window_height(self):
        theoretical_height = (CELL_HEIGHT + CELL_GAP) * (
            self.maxRow + 2
        )  # allows for row0 not counting
        ...
        print(
            f"current_screen:'{current_screen.name()}' screen_geometry:{screen_geometry} screen_height:{screen_height}"
        )

        window_geometry = self.geometry()
        current_window_height = window_geometry.height()
        print(
            f"current_window_height:{current_window_height} theoretical_height:{theoretical_height} self.maxRow:{self.maxRow}"
        )
        ...
        if current_window_height > theoretical_height:
            print(
                f"leaving window height as-is. current_window_height:{current_window_height} theoretical_height:{theoretical_height} screen_height:{screen_height}"
            )
            return

        print(
            f"target_window_height:{target_window_height} theoretical_height:{theoretical_height} screen_height:{screen_height}"
        )
...
if __name__ == "__main__":
    ...
    cell = controller.populate_cell(
        ...
        background_colour=Qt.GlobalColor.blue,
    ...
    controller.populate_cell(
        column=1,
        row=2,
        width=1,
        text_colour=Qt.GlobalColor.black,
        background_colour=Qt.GlobalColor.green,
        label="Stream1",
        dictionary={"description": "more"},
    )
    controller.populate_cell(
        column=1,
        row=3,
    ...
    controller.populate_cell(
        ...
        label="↘️",
        dictionary={"description": "Stream2 triggered by Task Task1a "},
    ),
    controller.populate_cell(
        column=2,
        row=5,
    ...
    def add_more_cells():
        controller.populate_cell(
            ...
            width=1,
            text_colour=Qt.GlobalColor.white,
            background_colour=Qt.GlobalColor.red,
            label="suprise!",
            dictionary={"description": "added after delay"},
        )
