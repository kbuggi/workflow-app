...
class Helper:
    ...
    def _get_int_from_dict(
        ...
        if not isinstance(item, int):
            logger.debug(
                f" found int {item} at  {location} in dictionary - though maybe it's just default {default} "
            )
            return item
        if item is None:
            logger.debug(
                f"didn't seem to find int at {location} in dictionary; returning default {default}  "
            )
            return default
        try:
            converted_item = int(item)
            logger.debug(
                f"converted {item} at {location} in dictionary to int; returning {converted_item}  "
            )
            return converted_item
        except ValueError as e:
            logger.warning(
                f"unable to convert {item} at {location} in dictionary to int; returning default {default}  "
            )
        return default
...
class Task:
    def __init__(self, name: str, dictionary: dict, parent_name: str = ""):
        ...
        self.name = name
        self.fullname = (
            str(parent_name) + "/" + name if parent_name != "" else name
        ...
        # User feedback - support minutes as a time
        if not self.duration:
            self.duration = 60 * Helper._get_int_from_dict(
                self.dictionary, level1="DurationMinutes", default=0
            )
        ...
        self.red = Helper._get_int_from_dict(self.dictionary, level1="Red", default=0)
...
class Checklist:
    def __init__(self, name, dictionary):
        ...
        self.title = name
        self.description = Helper._get_str_from_dict(
            self.dictionary, "Description", default=f"This is the {name} "
        )
...
class Stream:
    def __init__(self, name, dictionary):
        ...
        self.task_name_map = (
            {}
        )  # needed to check task names are unique within the stream and to enable x-stream linking
        ...
        self.build_warnings = (
            []
        )  # if we have any issues, a higher level UI can report to user
    ...
    def provide_dag(self, dag: SimpleDAG):
        if not isinstance(dag, SimpleDAG):
            raise TypeError(f"Expected SimpleDAG, got {type(SimpleDAG)}")
        self.dag = dag
    ...
    def resolve_tasks(self) -> list:
        ...
        for task_name in self.dictionary:
            ...
            if task_name == "Settings":
                ...
                task_dictionary = self.dictionary[
                    task_name
        ...
        if self.dag:
            self.update_dag_with_tasks(self.dag)
        return self.build_warnings

    def update_dag_with_tasks(self, dag):
        """adds stream and tasks to the DAG used for calculating critical path; call after resolve_tasks"""
        dag.add_node(self.name)
        task_previous = self.name
        for task in self.task_list:
            dag.add_node_following(task.fullname, task.duration, task_previous)
            task_previous = task.fullname

    def update_dag_with_triggered_streams(self, dag):
        """adds this stream's triggers to the DAG used for calculating critical path; call after resolve_triggered_streams"""
        for task in self.task_list:
            if not task.trigger_stream_list:
                continue
            for triggered_stream in task.trigger_stream_list:
                dag.add_edge(task.fullname, triggered_stream.name)
    ...
    def resolve_triggered_streams(self, stream_name_to_stream_reference_map) -> list:
        ...
        while task is not None:
            if task.trigger_stream_namelist:
                ...
                for trigger_stream_name in task.trigger_stream_namelist:
                    if (
                        trigger_stream_name
                        not in stream_name_to_stream_reference_map.keys()
                    ):
                        ...
                        trigger_stream = stream_name_to_stream_reference_map[
                            trigger_stream_name
                        ]
        ...
        if self.dag:
            self.update_dag_with_triggered_streams(self.dag)
        return warnings
    ...
    def iterator_visualiser(self, row, chain_set=set()):
        ...
        in_row = row
        warning = (
            f"display; Stream/self.name {self.name} row:'{row}' chain_set:{chain_set}"
        )
        logger.info(warning)
...
class WorkflowStream:
    def __init__(self, name, dictionary):
        ...
        self.media_file = Helper._get_str_from_dict(
            self.dictionary, "MediaFile", default=None
        )
        ...
        if "PreFlight" in self.dictionary:
            if not isinstance(self.dictionary["PreFlight"], dict):
                ...
                self.pre_checklist = Checklist(
                    "PreFlight_checklist", self.dictionary["PreFlight"]
                )
        if "PostFlight" in self.dictionary:
            if not isinstance(self.dictionary["PostFlight"], dict):
                ...
                self.post_checklist = Checklist(
                    "PostFlight_checklist", self.dictionary["PostFlight"]
                )
        ...
        for stream_name in self.dictionary["Streams"]:
            if not Helper._is_name_OK(stream_name):
                warning = (
                    f"Invalid Stream name '{stream_name}' in Workflow '{self.name}'"
                )
                logger.error(warning)
                raise ValueError(warning)
            if stream_name in self.stream_name_to_stream_reference_map:
                warning = (
                    f"Duplicate Stream name '{stream_name}' in Workflow '{self.name}'"
                )
                logger.error(warning)
                raise ValueError(warning)
    ...
    def build(self):
        ...
        #
        for stream in self.stream_list:
            w = stream.resolve_triggered_streams(
                self.stream_name_to_stream_reference_map
            )
            if w != []:
                warnings.append(w)  # TODO check warnings here?
    ...
    def calculate_critical_path_duration(self) -> int:
        if self.state != "Built":
            return -1
        return self.dag.critical_path_length()
        # return self.go_stream.calculate_critical_path_duration()

    def str_critical_path_all(self) -> int:
        if self.state != "Built":
            return "Not Built"
        return self.dag.str_critical_path_all()
    ...
    def _build_columns(
        ...
        for stream in self.stream_list:
            if stream.column is None or stream.column == "":
                ...
                warning = (
                    f"Stream {stream.name} has invalid column '{stream.column}' defined"
                )
                logger.error(warning)
                raise ValueError(warning)
...
if __name__ == "__main__":
    ...
    print(item)
    item = Helper._get_str_from_dict(
        dictionary, "info", "infomissing", default="defaulted"
    )
    print(item)
    ...
    print(
        f"*** Got {item} should be int {isinstance(item,int)} with matching default of -1 {item == -1} "
    )

    item = Helper._get_int_from_dict(dictionary, "score", default=-1)
    print(
        f"*** Got {item} should be int {isinstance(item,int)} with value 24 {item == 24} "
    )
    ...
    warnings = []
    file_name = r"resources/sample_recipes/recipe-simple.jsonc"
    try:
        ...
        if warnings != []:
            ...
            print(warnings)
        w.display()
        total = w.calculate_critical_path_duration()
        print(f"****** TOTAL : {total} *********")
        print(w.str_critical_path_all())
    ...
    print("----\nIn main\n")
    file_name = "resources/sample_recipes/recipe-eggs-and-soldiers-final.jsonc"
    try:
        with open(file_name, "r") as file:
            recipe_dict = jsonc.load(file)
        ...
        total = w2.calculate_critical_path_duration()
        print(f"****** TOTAL : {total} *********")
        print(w2.str_critical_path_all())
